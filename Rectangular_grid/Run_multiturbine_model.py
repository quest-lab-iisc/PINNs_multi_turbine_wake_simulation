...
from Rectangular_grid_config import PdeModel, get_ibc_and_inner_data
...
x = layers.Dense(units=700, activation='swish', kernel_initializer=initializer)(x)
x = layers.Dense(units=700, activation='swish', kernel_initializer=initializer)(x)
x = layers.Dense(units=700, activation='swish', kernel_initializer=initializer)(x)
...
cm = PdeModel(inputs=ivals, outputs=ovals, get_models=model_dict, loss_fn=loss_fn, optimizer=optimizer, metrics=metrics,
              parameters=parameters, batches=10, ibc_layer=False, mask=None)
