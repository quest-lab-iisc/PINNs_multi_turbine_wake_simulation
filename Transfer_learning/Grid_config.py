...
def get_ibc_and_inner_data(nu, u_ref, z_ref, TI_ref, Radius):
    ...
    disk_loc = [[0*Radius,-7*Radius,z_ref],[0*Radius,7*Radius,z_ref],[14*Radius,-7*Radius,z_ref],[14*Radius,7*Radius,z_ref]]
    ...
    for loc in disk_loc:
        ...
        indices_domain = np.where((RR<= 2*Radius) & (np.abs(np.abs(train_domain[:,0:1])-loc[0])<6*Radius))
    ...
    data_ponits =  data_ponits.iloc[::200]
    ...
    def act_disc_pde_residual(self, x_disk, y_disk, z_disk, disk_center,u_cl,c_t, training=True):
        ...
        with tf.GradientTape(persistent=True) as outer_tape:
            ...
            with tf.GradientTape(persistent=True) as inner_tape:
                ...
                k_zero_correction = .0001
                eps_zero_correction = .0001
                nu = 1.48e-5
