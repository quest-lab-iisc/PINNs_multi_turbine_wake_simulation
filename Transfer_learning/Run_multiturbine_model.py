...
from keras.initializers import GlorotUniform, GlorotNormal, HeNormal
...
from Grid_config import PdeModel, get_ibc_and_inner_data
...
u_ref=11.      #reference velocity/target velocity
z_ref=35.      #reference height
TI_ref=0.1     #reference turbulence intensity
Radius=15.05   #radius of turbine

Pre_trained_model_velocity = 9  #starting from a pre-trained model for velocity
...
#disk_center - center of the actuator disc
#x_da - coordinates of the data points
#inlet_uvwke, outlet_p, bottom_uvwke, top_uvwke, front_uvwke, back_uvwke - {boundary}_{boundary_values of different variables}

inlet, outlet, bottom, top, front, back, x_train, x_disk, disk_center, x_da, inlet_uvwke, outlet_p, bottom_uvwke, top_uvwke, front_uvwke, back_uvwke, da_uvwk = get_ibc_and_inner_data(nu=nu, u_ref=u_ref, z_ref=z_ref, TI_ref=TI_ref, Radius=Radius)
...
ivals = {'xin': x_train[:,0:1], 'yin': x_train[:,1:2], 'zin': x_train[:,2:3],
         ...
         'xd_da'    : x_da[:,0:1],      'yd_da'    : x_da[:,1:2],    'zd_da'   : x_da[:,2:3],
         'disk_center':  disk_center }

ovals = {'ub_inlet': inlet_uvwke[:,0:1], 'ub_bottom': bottom_uvwke[:,0:1], 'ub_top': top_uvwke[:,0:1], 'ub_front': front_uvwke[:,0:1], 'ub_back': back_uvwke[:,0:1], 'ub_da': da_uvwk[:,0:1],
           'vb_inlet': inlet_uvwke[:,1:2], 'vb_bottom': bottom_uvwke[:,1:2], 'vb_top': top_uvwke[:,1:2], 'vb_front': front_uvwke[:,1:2], 'vb_back': back_uvwke[:,1:2], 'vb_da': da_uvwk[:,1:2],
           'wb_inlet': inlet_uvwke[:,2:3], 'wb_bottom': bottom_uvwke[:,2:3], 'wb_top': top_uvwke[:,2:3], 'wb_front': front_uvwke[:,2:3], 'wb_back': back_uvwke[:,2:3], 'wb_da': da_uvwk[:,2:3],
           'pb': outlet_p[:,0:1], 
           'kb_inlet' : inlet_uvwke[:,3:4], 'kb_bottom' : bottom_uvwke[:,3:4], 'kb_top': top_uvwke[:,3:4], 'kb_front': front_uvwke[:,3:4], 'kb_back': back_uvwke[:,3:4], 'kb_da': da_uvwk[:,3:4],
...
model = tf.keras.models.load_model(f'Pre_trained/{Pre_trained_model_velocity}ms')
...
metrics = {"loss": keras.metrics.Mean(name='loss'),
           ...
           "residual_act_loss": keras.metrics.Mean(name='residual_act_loss'),
...
cm = PdeModel(inputs=ivals, outputs=ovals, get_models=model_dict, loss_fn=loss_fn, optimizer=optimizer, metrics=metrics,
              parameters=parameters, batches=16, ibc_layer=False, mask=None)



log_dir = 'log_output/'
history = cm.run(epochs=5000, proj_name="Actuator_multiturbine", log_dir=log_dir,
                 wb=False, verbose_freq=1000, plot_freq=1000)

cm.nn_model.save('out_hSaved_models/', save_format='tf')
